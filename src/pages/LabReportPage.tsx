...
const _trackedReportIds = new Set<string>();
...
export default function LabReportPage() {
  ...
  const reportKey = `${labReport.experimentName}_${labReport.date}`;
  useEffect(() => {
    if (_trackedReportIds.has(reportKey)) return;
    _trackedReportIds.add(reportKey);
    if (typeof pendo !== 'undefined') {
      pendo.track('lab_report_generated', {
        experimentName: labReport.experimentName,
        subjectName: labReport.subjectName,
        grade,
        date: labReport.date,
        hasObjective: !!labReport.objective,
        hasObservations: !!labReport.observations,
        hasAnalysis: !!labReport.analysis,
        hasConclusion: !!labReport.conclusion,
      });
    }
  }, [reportKey, labReport, grade]);
  ...
  const handlePrint = () => {
    if (typeof pendo !== 'undefined') {
      pendo.track('lab_report_downloaded', {
        experimentName: labReport.experimentName,
        subjectName: labReport.subjectName,
        grade,
        date: labReport.date,
