...
export default function VirtualLabPage() {
  ...
  const addItem = (kind: LabItem['kind']) => {
    ...
    pendo.track('virtual_lab_equipment_added', { equipmentType: kind, labMode });
  };

  const pour = (fromId: string, toId: string) => {
    if (fromId===toId) return;
    pendo.track('solution_poured_between_vessels', { labMode });
    setItems(prev => {
      ...
      for (const fc of from.reagents)
        for (const tc of to.reagents) {
          ...
          if (rxn) { pushReaction({ text:rxn.text, color:rxn.color, effect:rxn.effect }); break; }
        }
      return prev.map(v => {
      ...
      <header className="shrink-0 flex items-center gap-3 px-4 py-2.5 z-20"
        ...
        <div className="flex shrink-0 rounded-lg overflow-hidden" style={{ border:'1px solid #E7E5E0' }}>
          {(['chemistry','biology'] as const).map(mode => (
            <button key={mode} onClick={()=>{ pendo.track('virtual_lab_mode_switched', { from: labMode, to: mode }); setLabMode(mode); setOpenCategory(mode==='chemistry'?'Acids':'Stains'); setSelectedReagent(null); }}
