...
export default function LabBenchPage() {
  const { setScreen, selectedSubject, selectedExperiment,
    ...
    streamDrLab({
      ...
      onComplete: () => {
        ...
        window.pendo?.trackAgent("agent_response", {
          agentId: "5kFcAWbZk7LEGVKr_TxFSroTliM",
  ...
  const handleSend = useCallback(() => {
    ...
    const isSuggested = text === `What is the key principle of ${experiment?.name}?` ||
      text === 'How does this relate to real life?' ||
      text === 'What would happen if I changed a variable?';
    ...
    setMessages(prev => [...prev, {
      id: userMsgId, role: 'user', text, ts: now(), isAuto: false,
    }]);
    ...
    setInteractionCount(c => c + 1);
    incrementInteraction();
  }, [inputText, experiment, streamAI, incrementInteraction]);
